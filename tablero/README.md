...
> **Estado de la documentación:** este README cubre hasta **v105**. Para el detalle
...
| `tablero/CHANGELOG.md` | Detalle completo versión por versión (v1–v105 y siguientes). El README mantiene solo el resumen de hitos. |
| `README.md` | Este documento. |

> ⚠️ **Nota de desfase:** `DOCUMENTACION_TECNICA.md` describe la plataforma hasta **v74**
> y `MANUAL_USUARIOS.md` hasta **v94**, mientras que este README cubre hasta **v105**.
...
La Consola tiene **105 versiones** a esta fecha. Para no hacer eterno este documento, acá quedan
solo los **hitos importantes** — los cambios que movieron la aguja en arquitectura, seguridad
o cómo se usa la app día a día. El **detalle completo versión por versión** (v1–v105 y siguientes)
...
- **v104** — `/impeccable audit` sobre v99–v103: encontró y corrigió un hallazgo real de accesibilidad (los checkboxes de la vista previa de importación de horas eran nativos, sin `aria-label` y de 13px, por debajo del touch target mínimo); todo lo demás (gráficos, exports, uploader) dio limpio contra `DESIGN.md`.
- **v105** — `/impeccable polish`: ese checkbox nativo pasa a un botón `.rowSelectBtn` propio de la app (mismo idioma visual que el toggle ✓/✕ de GO Focus Plus/Surfsight), con color de señal en vez de nativo del navegador y actualización en el lugar al tildar/destildar filas.

---

*Para el detalle completo versión por versión (v1–v105 y siguientes), ver [`tablero/CHANGELOG.md`](tablero/CHANGELOG.md) y `git log`.*
